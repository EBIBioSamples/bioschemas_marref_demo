...
from biosamples.utilities import is_status, is_successful
...
parser.add_argument('biosamples_url', help='URL of BioSamples site')
...
config = dict({
    'biosamples_base_url': args.biosamples_url,
    'aap_username': AAP_USERNAME,
    'aap_password': AAP_PASSWORD,
    'curation_domain': 'self.MarRefCuration'
})
    ...
    with contextlib.closing(conn.cursor()) as curs:
        ...
        for row in rows:
            ...
            try:
                ...
                response = client.curate_sample(sample=sample, curation_object=curation_obj, jwt=jwt)
                if not is_successful(response):
                    raise Exception("An error occurred while curating sample {} - HTTP Status {}".format(
